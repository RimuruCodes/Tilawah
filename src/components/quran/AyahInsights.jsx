        ...
        {open === "tafsir" && (
          ...
          >
            <div className="rounded-xl bg-slate-800/30 border border-slate-700/30 p-4 space-y-3">
              {tafsir === undefined && (
                <div className="flex justify-center py-4"><Loader2 className="w-4 h-4 text-emerald-400 animate-spin" /></div>
              )}
              {tafsir === null && (
              ...
              {typeof tafsir === "string" && (
                <>
                  <div className="text-sm text-slate-300 leading-relaxed whitespace-pre-line max-h-80 overflow-y-auto pr-1">
                    {tafsir}
                  </div>
                  <p className="text-[10px] text-slate-600 border-t border-slate-700/30 pt-2">
        ...
        {open === "words" && (
          ...
          >
            <div className="rounded-xl bg-slate-800/30 border border-slate-700/30 p-4 space-y-3">
              ...
              {Array.isArray(words) && (
                <>
                  <p className="text-[10px] text-slate-600">Tap a word for its meaning.</p>
                      ...
                      <button
                        ...
                        className={`px-2.5 py-1.5 rounded-lg font-arabic text-lg leading-normal transition-colors border ${
                          activeWord === i
                            ...
                            : "bg-slate-800/50 border-slate-700/30 text-white/90 hover:border-slate-600/50"
                        }`}
                      >
                        {w.arabic}
                      </button>
                  ...
                  {activeWord != null && words[activeWord] && (
                    <div className="rounded-lg bg-slate-900/60 border border-emerald-500/20 p-3 space-y-1">
                      <p dir="rtl" className="font-arabic text-xl text-emerald-200">{words[activeWord].arabic}</p>
                      {words[activeWord].transliteration && (
                        <p className="text-xs text-slate-400 italic">{words[activeWord].transliteration}</p>
                      )}
                      <p className="text-sm text-slate-200">{words[activeWord].meaning || "—"}</p>
                    </div>
                  )}
                  <p className="text-[10px] text-slate-600 border-t border-slate-700/30 pt-2">
